...
struct MainCatalogView: View {
    ...
    @State private var viewMode: ViewMode = .word
    ...
    func matchType(for item: SketchItem, searchText: String) -> SearchMatchType {
        let bookMatch = item.bookName?.localizedCaseInsensitiveContains(searchText) ?? false
        let verseMatch = "\(item.chapter):\(item.verse)".contains(searchText)

        if bookMatch || verseMatch {
            return .scripture
        } else {
            return .word
        }
    ...
    var scriptureMatches: [SketchItem] {
        guard !searchText.isEmpty else { return [] }
        return filteredItems.filter { matchType(for: $0, searchText: searchText) == .scripture }
    }

    var wordMatches: [SketchItem] {
        guard !searchText.isEmpty else { return [] }
        return filteredItems.filter { matchType(for: $0, searchText: searchText) == .word }
    }

    @ViewBuilder
    var scriptureResultsView: some View {
        let items = searchText.isEmpty ? filteredItems : scriptureMatches
    ...
    @ViewBuilder
    var wordResultsView: some View {
        let items = searchText.isEmpty ? filteredItems : wordMatches
        let grouped = Dictionary(grouping: items) { $0.centerWord ?? "Unknown" }
        ...
        NavigationStack {
            Group {
                if allItems.isEmpty {
                    ...
                    List {
                        if !searchText.isEmpty {
                            // Smart search results: show matches organized by type
                            if !scriptureMatches.isEmpty {
                                Section(header: Text("Scripture References")) {
                            ...
                            // Normal mode: show based on selected view mode
